...
def main():
    ...
    try:
        with create_db_session() as session:
            ...
            city, edges_df = get_or_create_city(session, city_name=CITY_NAME,
            ...
            run_config = get_or_create_run_config(session, city_id=city.city_id,
            ...
            clusters = get_clusters_by_connectivity(congestion_df, resolution=CLUSTER_RESOLUTION,
            ...
            qa_assignment, gurobi_assignment, all_filtered_ids, affected_edges_df = process_clusters(
                session, run_config=run_config, iteration_id=iteration_id, route_alternatives=route_alternatives,
                vehicle_routes_df=routes_df, weights_df=weights_df,
                duration_penalty_df=duration_penalty_df, clusters=clusters, cluster_resolution=CLUSTER_RESOLUTION, comp_type=COMP_TYPE, qubo_output_dir= Path(MATRIX_OUTPUT_DIR))

            affected_df = pd.concat(affected_edges_df, ignore_index=True) if affected_edges_df else pd.DataFrame(columns=['edge_id', 'congestion_score', 'cluster_id'])
            optimized_vehicle_ids = [vid for sublist in all_filtered_ids for vid in sublist]

            post_qa_df, _ = post_qa_congestion(session, run_configs_id=run_config.run_configs_id,
                                               iteration_id=iteration_id, all_vehicle_ids=all_vehicle_ids,
                                               optimized_vehicle_ids=optimized_vehicle_ids, qa_assignment=qa_assignment,
                                               method=ROUTE_METHOD)

            post_gurobi_df, _ = post_gurobi_congestion(session, run_configs_id=run_config.run_configs_id,
                                                       iteration_id=iteration_id, all_vehicle_ids=all_vehicle_ids,
                                                       optimized_vehicle_ids=optimized_vehicle_ids,
                                                       gurobi_assignment=gurobi_assignment, route_alternatives=route_alternatives,
                                                       method=ROUTE_METHOD)
            ...
            all_scores = pd.concat([
                shortest_dur_df['congestion_score'],
                shortest_dis_df['congestion_score'],
                post_qa_df['congestion_score'],
                random_df['congestion_score'],
                post_gurobi_df['congestion_score']
            ])
            ...
            save_congestion_summary(session, run_config_id=run_config.run_configs_id,
                                     iteration_id=iteration_id, edges_df=edges_df,
                                     congestion_df=congestion_df, post_qa_df=post_qa_df,
                                     shortest_dur_df=shortest_dur_df, shortest_dis_df=shortest_dis_df,
                                     random_df=random_df, post_gurobi_df=post_gurobi_df)

            run_sql_query(session, run_config_id=run_config.run_configs_id,
                          iteration_id=iteration_id, sql_name='congestion_results.sql')
